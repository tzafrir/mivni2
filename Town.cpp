...
Town::TownResult Town::AddNeighborhood(int population) {
	...
	if (Neighborhoods < ma) {
		if (NeighborhoodsBottom.insert(population) == Failure) {
			return TownFailure;
		}
		Neighborhoods++;
		return TownSuccess;
	...
	if (NeighborhoodsTop.insert(-1 * population) == Failure) { 
		return TownFailure;
	}
	Neighborhoods++;
	return TownSuccess;
...
Town::TownResult Town::AddManyNeighborhoods(int size, const int* populations) {
	...
	if (Neighborhoods + size <= ma) {
		if (NeighborhoodsBottom.makeHeap(tmp, Neighborhoods+size) == Failure) {
			delete[] tmp;
			return TownFailure;
		}
	} else {
		if (NeighborhoodsBottom.makeHeap(tmp, ma) == Failure) {
			delete[] tmp;
			return TownFailure;
		}
		...
		if (NeighborhoodsTop.makeHeap(&tmp[ma], Neighborhoods + size - ma) == Failure) {
			delete[] tmp;
			return TownFailure;
		}
	}
